...
import type { BiddingItem, PrismaClient } from '@prisma/client';
...
export class ParticipationService {
  ...
  async updateItems(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await prisma.participationItem.deleteMany({ where: { participationId } });

      for (const item of dto.items) {
        const biddingItem = biddingItems.find((bi: BiddingItem) => bi.id === item.biddingItemId);
        ...
        await prisma.participationItem.create({
          data: {
            participationId,
