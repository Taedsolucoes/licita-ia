...
export class MatchingService {
  ...
  async matchBiddingForAllTenants(biddingId: string): Promise<void> {
    ...
    // Dispatch analysis job once per bidding after matching
    if (matched > 0) {
      try {
        ...
        this.logger.log(`Queued Claude analysis for bidding ${biddingId}`);
      } catch (analysisError) {
        this.logger.error(
          `Failed to queue analysis for bidding ${biddingId}: ${analysisError instanceof Error ? analysisError.message : String(analysisError)}`,
        );
      }
    }
  }
